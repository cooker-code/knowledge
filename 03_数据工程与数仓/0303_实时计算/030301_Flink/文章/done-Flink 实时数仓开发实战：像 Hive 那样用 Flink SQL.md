...
> 已吸收至：[[03_数据工程与数仓/0303_实时计算/030301_Flink/030301_核心知识点/FlinkSQL脚本化提交与Application模式适配|Flink SQL 脚本化提交与 Application 模式适配]]
...
**方式一：[SQL Client](https://zhida.zhihu.com/search?content_id=276653367&content_type=Article&match_order=1&q=SQL+Client&zhida_source=entity) 交互模式**
...
社区也意识到了不能在 Application Mode 下提交 SQL 脚本的痛点。[FLIP-316](https://link.zhihu.com/?target=https%3A//cwiki.apache.org/confluence/display/FLINK/FLIP-316%253A%2BSupport%2Bapplication%2Bmode%2Bfor%2BSQL%2BGateway) 和 [FLIP-480](https://link.zhihu.com/?target=https%3A//cwiki.apache.org/confluence/display/FLINK/FLIP-480%253A%2BSupport%2Bto%2Bdeploy%2BSQL%2Bscript%2Bin%2Bapplication%2Bmode) 是社区在这个方向上的两代方案。
...
下面将以开源实现 [flink-sql-bootstrap](https://link.zhihu.com/?target=https%3A//github.com/tonyabasy/flink-sql-bootstrap) 为例讲解使用方式、最佳实践、原理剖析、自建指南。你可以参考其源码自建，也可以直接使用。
...
-   **HTTP/HTTPS**：通常用于某个后端服务集成或 Git 服务器，例如：`https://my-server/example-word-count.sql`
...
_注：其他参数及更多使用方式可以通过_ `_--help_` _查阅，也可以项目的_ [README](https://link.zhihu.com/?target=https%3A//github.com/tonyabasy/flink-sql-bootstrap/blob/main/README_CN.md)_。_
...
完整的最佳实践示例工程见 [flink-sql-bootstrap-examples](https://link.zhihu.com/?target=https%3A//github.com/tonyabasy/flink-sql-bootstrap-examples) 仓库。
...
![](Flink_实时数仓开发实战_像_Hive_那样用_Flink_SQL_assets/img_001.jpg)

图 1 · Flink 2.x SQL 脚本 Application Mode 完整执行链路
...
![](Flink_实时数仓开发实战_像_Hive_那样用_Flink_SQL_assets/img_002.jpg)

图 2 · 官方方案（Flink 2.x）与本文方案（Flink 1.20+）核心差异
...
![](Flink_实时数仓开发实战_像_Hive_那样用_Flink_SQL_assets/img_003.jpg)

图3 · 三阶段处理 Multi-Statement SQL
...
| org.apache.flink:flink-table-runtime | Planner, [TableEnvironmentImpl](https://zhida.zhihu.com/search?content_id=276653367&content_type=Article&match_order=1&q=TableEnvironmentImpl&zhida_source=entity) |
...
**① `SessionContext` 中的 URI → URL 转换 Bug（**[FLINK-39687](https://link.zhihu.com/?target=https%3A//issues.apache.org/jira/browse/FLINK-39687)**）**
...
> 参考：`[UriSafeSessionContext.java](https://link.zhihu.com/?target=https%3A//github.com/tonyabasy/flink-sql-bootstrap/blob/main/src/main/java/com/lanting/flink/sql/bootstrap/flink/UriSafeSessionContext.java)`
...
> 参考：`[ApplicationOperationExecutor.java](https://link.zhihu.com/?target=https%3A//github.com/tonyabasy/flink-sql-bootstrap/blob/main/src/main/java/com/lanting/flink/sql/bootstrap/flink/ApplicationOperationExecutor.java)`

**③ 执行结果打印适配**

Flink 2.x 在 `ResultFetcher` 中新增了 `getPrintStyle()` 方法，1.20 中不存在。自建 `Printer`，直接从 `ResolvedSchema` 构建 `TableauStyle` 来格式化输出。

> 参考：`[Printer.java](https://link.zhihu.com/?target=https%3A//github.com/tonyabasy/flink-sql-bootstrap/blob/main/src/main/java/com/lanting/flink/sql/bootstrap/executor/Printer.java)`

### 3\. 构建提交流程

依赖和兼容性处理完毕后，提交流程的核心骨架如下。代码中标注了哪些是 Flink 自带的类、哪些需要自建：

> 参考：`[SqlEntryPoint.java](https://link.zhihu.com/?target=https%3A//github.com/tonyabasy/flink-sql-bootstrap/blob/main/src/main/java/com/lanting/flink/sql/bootstrap/SqlEntryPoint.java)`
...
-   [FLIP-316: Support Application Mode for SQL Gateway](https://link.zhihu.com/?target=https%3A//cwiki.apache.org/confluence/display/FLINK/FLIP-316%253A%2BSupport%2Bapplication%2Bmode%2Bfor%2BSQL%2BGateway)
-   [FLIP-480: Support to Deploy SQL Script in Application Mode](https://link.zhihu.com/?target=https%3A//cwiki.apache.org/confluence/display/FLINK/FLIP-480%253A%2BSupport%2Bto%2Bdeploy%2BSQL%2Bscript%2Bin%2Bapplication%2Bmode)
-   [Flink 2.2 SQL Gateway — Deploying a Script](https://link.zhihu.com/?target=https%3A//nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql-gateway/overview/)
-   [Flink 2.0 Release Notes](https://link.zhihu.com/?target=https%3A//nightlies.apache.org/flink/flink-docs-stable/release-notes/flink-2.0/)
-   [FLINK-39687: URI used as URL in SessionContext causes ArrayStoreException](https://link.zhihu.com/?target=https%3A//issues.apache.org/jira/browse/FLINK-39687)

_本文基于_ [github - flink-sql-bootstrap](https://link.zhihu.com/?target=https%3A//github.com/tonyabasy/flink-sql-bootstrap) _&_ [gitee - flink-sql-bootstrap](https://link.zhihu.com/?target=https%3A//gitee.com/tonyabasy/flink-sql-bootstrap)
...
-   项目中提交 [Issue](https://link.zhihu.com/?target=https%3A//github.com/tonyabasy/flink-sql-bootstrap/issues)
-   Email：[tonyabasy@163.com](mailto:tonyabasy@163.com)

👏 同时欢迎大家参与 [flink-sql-bootstrap](https://link.zhihu.com/?target=https%3A//github.com/tonyabasy/flink-sql-bootstrap) 共建。

编辑于 2026-06-11 22:28・北京

[

Flink

](https://www.zhihu.com/topic/20043072)
