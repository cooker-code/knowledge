> 已吸收至：[[03_数据工程与数仓/0303_实时计算/030301_Flink/030301_核心知识点/StateProcessorAPI离线状态读写|StateProcessorAPI离线状态读写]]
---
title: State Processor API：如何读写和修改 Flink 应用程序的状态
author: 大数据生态
date:
url: http://mp.weixin.qq.com/s?__biz=MzI0OTYwNTQ4Ng==&mid=2247485618&idx=1&sn=1c3ac6e0173fb3c9a52ffda9c12dda6a&chksm=e98fb4b3def83da53f263f1b0c47a737a72a9880c4c7ffa53dc2bf4ed60fb15a71e55fb1a3c6&mpshare=1&scene=24&srcid=0305PwyiRlPjx0Gq5QZw6Dbg&sharer_sharetime=1646439963490&sharer_shareid=4145ee29355a80b1b2b01921ded70e3a#rd
---
...
State Processor API 现在提供了创建、加载和写入 Savepoint 的方法。你可以从加载的 Savepoint 上读取数据集或者将数据集转换为状态并将其添加到 Savepoint 中。可以使用 DataSet API 的完整功能来处理数据集。使用这些方法，可以解决所有前面提到的用例（以及更多用例）。如果您想详细了解如何使用 State Processor API，请查看文档：https://ci.apache.org/projects/flink/flink-docs-release-1.9/dev/libs/state\_processor\_api.html

## 4. 为什么使用 DataSet API？

如果你对 Flink 的路线图比较熟悉，你可能会惊讶 State Processor API 为什么要基于 DataSet API 来构建。因为 Flink 社区正计划使用 BoundedStreams 的概念扩展 DataStream API，并弃用 DataSet API。在设计此功能时，我们评估了 DataStream API 以及 Table API，但都无法提供相应功能的支持。我们不想因为 Flink API 的进展而阻塞这个特性的开发，因此我们决定先在 DataSet API 上构建这个功能，但将其对 DataSet API 的依赖降到最低。因此，将其迁移到另一个 API 也相当容易。
...
原文:The State Processor API: How to Read, write and modify the state of Flink applications