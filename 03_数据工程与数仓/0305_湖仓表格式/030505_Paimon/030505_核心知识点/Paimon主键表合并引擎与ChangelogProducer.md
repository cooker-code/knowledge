...
- 本地文件 1：[Paimon 合并引擎与 Changelog Producer 最优搭配指南](<../文章/done-Paimon 合并引擎与 Changelog Producer 最优搭配指南.md>)
- 本地文件 2：[Paimon 合并引擎到底怎么选？一文讲透 Deduplicate、Partial Update、Aggregation](<../文章/done-Paimon 合并引擎到底怎么选？一文讲透 Deduplicate、Partial Update、Aggregation.md>)
...
| 记住 | Input 低延迟但依赖上游 CDC 语义正确 | 上游质量直接决定下游正确性 | 需要验证 Debezium/030302_Flink CDC 事件类型 |
...
| 来源 | 认知增量 | 处理 |
|---|---|---|
| [[03_数据工程与数仓/0305_湖仓表格式/030505_Paimon/文章/done-Paimon系列：主键表之合并引擎merge-engine]] | 补充该主题的生产案例、机制边界或排重样例。 | 重新判断后补入目标知识产物 |
| [[03_数据工程与数仓/0305_湖仓表格式/030505_Paimon/文章/done-阿里面试：Paimon Changelog 和 合并引擎有哪些组合，分别适用于哪些场景？]] | 补充该主题的生产案例、机制边界或排重样例。 | 重新判断后补入目标知识产物 |
