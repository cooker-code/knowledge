...
author: 阿里云开发者
date: 
url: http://mp.weixin.qq.com/s?__biz=MzIzOTU0NTQ0MA==&mid=2247538350&idx=1&sn=be56a893b0016c6e32ffe9ee2eb9a890&chksm=e92a6ba1de5de2b7ff7f09b71a8dce59e5ffc2a01736dc39777f587ce359ba6ae00ff9a157fa&mpshare=1&scene=24&srcid=0428bHcu3YEHxyEobkFVSC7v&sharer_shareinfo=0517dbd1a725f151cb5cf72c9e967241&sharer_shareinfo_first=0517dbd1a725f151cb5cf72c9e967241#rd
---
...
```
SELECT  prize_id        ,COUNT(*) AS prize_send_cnt_1dFROM    apcdm.dwd_ap_mkt_eqt_send_diWHERE   dt = '${bizdate}'AND     prize_id IN ('PZ169328936', 'PZ169298703')GROUP BY prize_id;
```
...
| 调度参数 | Dataphin任务调度时使用的参数，根据任务调度的业务日期、定时时间及参数的取值格式自动替换为具体的值，实现在任务调度时间内参数的动态替换。  业务日期：调度日期前一天  调度/定时时间：任务实例计划运行的时间 |
...
通过将项目空间设置到某个资源组，这个项目空间提交的任务就能够使用这个资源组的槽位，每个资源组通过设置最大槽位(max\_slot)来控制并发，可通过tesla实时查看项目空间的槽位使用情况。
