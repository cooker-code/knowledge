...
---
title: 一篇文章教会你数据仓库之详解拉链表怎么做！
author: 五分钟学大数据
date:
url: http://mp.weixin.qq.com/s?__biz=Mzg2MzU2MDYzOA==&mid=2247503425&idx=1&sn=7e8a69416cc20c5e244c9ffe1044ad04&chksm=ce743890f903b1865c9a2a4cfe9ae0b5be0ce111cafac830641e7423a8d28dc4e2e8ec9e7aaa&mpshare=1&scene=24&srcid=12223KDLWfYM6JtYfcx3zJqp&sharer_shareinfo=64b184b2eb8e123f64e8430e81dccbbb&sharer_shareinfo_first=64b184b2eb8e123f64e8430e81dccbbb#rd
---

发现一篇好文章，读完感觉写的不错，对数据仓库拉链表的设计和开发写的比较详细，分享给大家。(ง•̀\_•́)ง
...
* t\_start\_date表示该条记录的生命周期开始时间，t\_end\_date表示该条记录的生命周期结束时间。
* t\_end\_date = '9999-12-31'表示该条记录目前处于有效状态。
* 如果查询当前所有有效的记录，则select \* from user where t\_end\_date = '9999-12-31'。
* 如果查询2017-01-02的历史快照，则select \* from user where t\_start\_date <= '2017-01-02' and t\_end\_date >= '2017-01-02'。（**此处要好好理解，是拉链表比较重要的一块**。）
...
1. 在一些查询引擎中，我们对start\_date和end\_date做索引，这样能提高不少性能。
...
1. 使用拉链表的时候可以不加t\_end\_date，即失效日期，但是加上之后，能优化很多查询。
