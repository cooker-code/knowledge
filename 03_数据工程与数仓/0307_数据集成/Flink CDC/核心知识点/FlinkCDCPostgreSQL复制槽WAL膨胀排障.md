...
| 项 | 内容 |
|---|---|
| 用户当前认知层级 | Flink CDC / 数据集成 L2-L3 draft |
| 认知成熟度 | draft |
| 阅读投入建议 | 精读 |
...
| 冲突类型 | 具体表现 | 影响 | 处理 |
|---|---|---|---|
| 原目录冲突 | 原文在 raws/big-data，脚本粗分到实时计算 | 容易把问题看成 Flink 运行问题 | 重路由到数据集成 / Flink CDC |
| 证据不足 | 数字很具体，但缺 PostgreSQL/Flink CDC 版本、日志和变更审批记录 | 不能直接当通用 SOP | 保留机制和检查项，不沉淀具体阈值为标准 |
...
| 门槛 | 判断 | 证据 |
|---|---|---|
| 可运行 | 部分 | 有 SQL，但无完整 PostgreSQL/Flink CDC 环境 |
