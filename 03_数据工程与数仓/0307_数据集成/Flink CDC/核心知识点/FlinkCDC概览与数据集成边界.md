...
|---|---|---|---|
| Flink CDC 的本体是数据集成 | 它捕获数据库变更并同步下游，不是通用流计算机制 | 纠偏分类边界 | 新建数据集成/Flink CDC |
...
| 冲突类型 | 具体表现 | 影响 | 处理 |
|---|---|---|---|
| 关键词误导 | 标题含 Flink，容易归入实时计算 | 会混淆采集和计算职责 | 重路由到数据集成 |
...
| 分级 | 内容 | 为什么值得吸收 | 后续动作 |
|---|---|---|---|
| 理解 | CDC 捕获 insert/update/delete 变更，是实时数仓入口 | 明确技术本体 | 写入数据集成 index |
