...
| 路线 | 流程入口 | 当前文章数 | 当前状态 |
|---|---|---:|---|
| 接口契约与网关 | [接口契约与网关/AGENTS.md](接口契约与网关/AGENTS.md) | 5 | 覆盖 AI Gateway、API 工具、接口优化、SPI/API 边界；需要补契约测试和错误语义 |
| 测试质量 | [测试质量/AGENTS.md](测试质量/AGENTS.md) | 10 | 覆盖 AI 测试技能、流量回放、测试报告、决策表、压测和测试基础设施；当前是最高优先级精读池 |
| 安全权限 | [安全权限/AGENTS.md](安全权限/AGENTS.md) | 2 | 覆盖 LDAP 高可用集成和 GitHub 2FA；需要补威胁模型、审计和凭证治理 |
| 部署发布与环境 | [部署发布与环境/AGENTS.md](部署发布与环境/AGENTS.md) | 2 | 覆盖 DockerHub 拉取和 Docker 策略变化；需要补镜像、配置、回滚和环境可复现 |
| 可观测性与 AIOps | [可观测性与AIOps/AGENTS.md](可观测性与AIOps/AGENTS.md) | 7 | 覆盖日志、Prometheus/PromQL、RCA、AIOps；已有极简异常检测核心知识点 |
| 并发运行时 | [并发运行时/AGENTS.md](并发运行时/AGENTS.md) | 1 | 当前只有 ThreadPoolExecutor；后续补线程池、事件循环、协程和资源隔离 |
...
| 认证、授权、2FA、LDAP、凭证、审计、权限边界 | `安全权限` | Agent 沙箱权限应进 `02_Agent与AI工程/0208_安全与权限` |
...
| 电脑工具 / 待人工判断 | VSCode、Visual Studio、JupyterLab、打工助手迁到电脑工具；DataFun 知识地图进待人工判断 |
