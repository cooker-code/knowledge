...
- 本模块先看宏观流程，再看文章：[知识地图](070105_核心知识点/知识地图.md)。
- 新文章必须先归入流程节点，再判断是补充、冲突、不同层次还是降权。
- `文章/` 只保留原文锚点，长期知识必须沉淀到 `070105_核心知识点/`。
...
| Spring 生态能力在 Spring Boot 应用中的落地，例如 Spring Batch、Resilience4j、HikariCP、EasyExcel、MinIO、Tika、Calcite | 跨语言测试平台、接口契约治理、压测体系、安全权限和可观测性；这些进入 `0703_工程实践与质量保障` |
| 以 Spring Boot 为主体的低代码平台或业务案例 | 纯前端框架、数据分析方法、智能问数和调度编排平台 |
...
| S03 数据访问与文件集成 | 连接池、Excel、对象存储、文档解析、多源查询如何接入 | 5 | 区分框架集成教程和可复用工程边界 |
...
| S08 低代码/AI/业务案例 | 低代码平台、DL4J 业务例子是否值得沉淀 | 2 | 默认略读，只有形成架构边界或验证动作才精读 |

## 已沉淀核心知识点入口

当前 Spring Boot 目录先承接文章锚点和流程路由。已有 Java 目录中与 Spring Boot 强相关的判断，后续精读时应迁移或重写到本目录，例如：
...
| 数据源、连接池、Excel、MinIO、Tika、Calcite | S03 | 是否有资源生命周期、事务、容量和失败模式 |
| 批处理、规则引擎、动态插件 | S05 | 是否可重启、可追踪、可审计、可卸载 |
| 性能优化 | S06 | 是否给出基线、环境、指标和副作用 |
| AI/低代码业务案例 | S08 | 是否只是示例包装，还是能沉淀通用架构规则 |
