...
author: 翊行代码
date:
url: https://mp.weixin.qq.com/s?__biz=MjM5NTQ3NDAwMw==&mid=2452917957&idx=1&sn=9d1b61245fd1e0b55995026a4d866759&chksm=b0554c3fd673c1ea3a8b03f441106d0c3effb3775a1f9b1f77077c90e6c2187d2942a90201a4&mpshare=1&scene=24&srcid=1211KONceO61HF654TcicDOm&sharer_shareinfo=5e91c8977a467f2c79b5c91bae1558a4&sharer_shareinfo_first=5e91c8977a467f2c79b5c91bae1558a4#rd
---
...
1. 1. 10 行代码接入一个数学 MCP 服务，体验最小闭环
2. 2. 把多个 MCP 服务拼成一个“工具中枢”，让 Agent 像用本地工具一样用它们
3. 3. 给高危 MCP 工具加上 LangGraph checkpointer + Human-in-the-Loop，做到“可暂停、可审批、可追责”
...
create_agent+ middleware+ checkpointer
...
## 三、Part 1：10 行代码把数学 MCP 接入 create\_agent
...
    asyncwith client.session("math") as session:
...
@mcp.tool()
asyncdefget_weather(location: str) -> str:
    """Get weather for location."""
    # 教学示例，真实场景可以接第三方天气 API
    returnf"The weather in {location} is always sunny in this demo."
...
    # 演示：从多个服务器加载工具并合并
    all_tools = []
    asyncwith client.session("math") as math_session:
        all_tools.extend(await load_mcp_tools(math_session))

    asyncwith client.session("weather") as weather_session:
...
    asyncwith client.session("shell") as shell_session:
...
    ifnot first_result.get("interrupts"):
        print("未触发任何中断。")
        return
...
```
人类审批者MCP Shell ServerCheckpointerHITL MiddlewareAgent（create_agent）用户/业务代码人类审批者MCP Shell ServerCheckpointerHITL MiddlewareAgent（create_agent）用户/业务代码ainvoke(messages, thread_id)计划调用 run_shell_command保存当前状态并触发 interrupt返回 interrupts 信息展示待审批工具调用approve / edit / rejectCommand(resume={decisions}, thread_id)恢复中断前状态根据决策调用 MCP 工具返回工具结果返回最终回答
```
...
## 六、从第 08 篇（create\_react\_agent）迁到 v1 的实战攻略
