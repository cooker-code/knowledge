...
author: NotesOnAI
date: 
url: https://mp.weixin.qq.com/s?__biz=MzkyODY1NzAyMQ==&mid=2247484724&idx=1&sn=60cf81fba10a196d2a40b81db3dffe94&chksm=c39889781394f78a32bb58aeeb6b4752262f2a5909b0cfeb339f9bbefe043749f363c4321d0b&mpshare=1&scene=24&srcid=0919rlWoNx6GvSX8wlGC8Nj8&sharer_shareinfo=36f26cdefd0bdfa8fa3d9eb5aa5e7519&sharer_shareinfo_first=36f26cdefd0bdfa8fa3d9eb5aa5e7519#rd
---
...
> 由于平台限制，本文大多数链接无法直接跳转，如果对蓝色链接感兴趣，可以访问原文：   
> https://github.com/Hoper-J/AI-Guide-and-Demos-zh\_CN/blob/master/Guide/深入%20FastMCP%20源码：认识%20tool()、resource()%20和%20prompt()%20装饰器.md
...
    """返回支持查询天气的城市列表"""  
    cities = ["北京", "上海", "广州", "深圳"]  
    returnf"Cities: {', '.join(cities)}"  
  
@mcp.resource("resource://{city}/weather")  
defget_weather(city: str) -> str:  
    returnf"Weather for {city}"  
...
接下来会着重讲解 tool() 装饰器，resource() 和 prompt() 的处理逻辑基本是 tool() 的简化版本。
...
1. 1. 自动提取函数的参数类型信息（下面的信息由之后的 `debug_func_metadata` 打印），以及文档字符串。
   ...
   ```
2. 2. 生成参数的 JSON Schema（函数名+Arguments）。
   ...
   ```
3. 3. 将函数注册为 MCP 工具（`self._tools[tool.name] = tool`）。

另外，`mcp.tool()` 可以接受以下参数（此处参数解释参考源代码的 tool 和 func\_metadata）：

* • **name**: 可选的工具名称，默认为函数名
* • **title**: 可选的工具标题（用于人类阅读）
* • **description**: 可选的工具功能描述，默认使用函数的文档字符串
* • **annotations**: 可选的 ToolAnnotations，提供额外的工具信息
* • **structured\_output**：控制工具输出是结构化还是非结构化的
...
        if tools isnotNone:  
            for tool in tools:  
                if warn_on_duplicate_tools and tool.name inself._tools:  
...
        if existing:  
            ifself.warn_on_duplicate_tools:  
                logger.warning(f"Tool already exists: {tool.name}")  
            return existing  
        self._tools[tool.name] = tool  
        return tool
...
1. 1. **装饰器检查调用方式是否正确（必须带括号），然后将被装饰的函数传递给 `ToolManager.add_tool()`**。
   ...
               return decorator
   ```
2. 2. **`Tool.from_function()` 处理函数元数据**。
...
1. `tool = Tool.from_function(...)` 是当前最重要的处理部分，对应代码位于 tools/base.py，其主要步骤如下（代码按顺序拼接等价于 Tool.from\_function()，出于讲解目的将其进行了拆分）：
   ...
   **b. 自动检测 Context 参数**， `inspect.signature()` 会遍历函数的所有参数，检查参数类型是否为 Context 的子类，如果是的话会记录为 `context_kwarg`，这个参数会被传入 `func_metadata()` 的 `skip_names`（可以跳过这一步的理解，等真正涉及到的时候再探究），不会出现在工具对应的 JSON Schema 中。
   ...
           ...  
           # 自动检测 Context 参数  
           if context_kwarg isNone:  
               sig = inspect.signature(fn)  
               for param_name, param in sig.parameters.items():  
                   # 跳过泛型类型  
                   if get_origin(param.annotation) isnotNone:  
                       continue  
                   # 检查参数类型是否是 Context 的子类  
                   ifissubclass(param.annotation, Context):  
   ...
   **c. 生成参数的 JSON Schema**：
   ...
               fn,  
               skip_names=[context_kwarg] if context_kwarg isnotNoneelse [],  
               structured_output=structured_output,  
           )  
   ...
   `func_metadata` 相关源代码位于 utilities/func\_metadata.py，这里我们进行主体逻辑的抽取打印（完整 debug\_func\_metadata 函数见附录）：
...
`ResourceManager` 的实现位于 resources/resource\_manager.py：

```
class ResourceManager:  
    """管理 FastMCP 资源。"""  
  
    def__init__(self, warn_on_duplicate_resources: bool = True):  
        self._resources: dict[str, Resource] = {}  
        self._templates: dict[str, ResourceTemplate] = {}  
        self.warn_on_duplicate_resources = warn_on_duplicate_resources  
  
    defadd_resource(self, resource: Resource) -> Resource:  
        """向管理器中添加资源。  
          
        参数：  
...
        if existing:  
            ifself.warn_on_duplicate_resources:  
...
1. 1. **静态**：`FunctionResource` 位于 resources/types.py：
   ...
           )
   ```
2. 2. **动态**：`ResourceTemplate` 位于 resources/templates.py：
...
        defdecorator(func: AnyFunction) -> AnyFunction:  
...
`PromptManager` 的实现位于 prompts/prompt\_manager.py：

```
class PromptManager:  
    """管理 FastMCP 提示词。"""  
      
    def__init__(self, warn_on_duplicate_prompts: bool = True):  
        self._prompts: dict[str, Prompt] = {}  
        self.warn_on_duplicate_prompts = warn_on_duplicate_prompts  
          
    defadd_prompt(self, prompt: Prompt) -> Prompt:  
        """添加提示词到管理器。"""  
        logger.debug(f"Adding prompt: {prompt.name}")  
        existing = self._prompts.get(prompt.name)  
        if existing:  
            ifself.warn_on_duplicate_prompts:  
                logger.warning(f"Prompt already exists: {prompt.name}")  
            return existing  
        self._prompts[prompt.name] = prompt  
        return prompt  
  
    defget_prompt(self, name: str) -> Prompt | None:  
        """根据名称获取提示词。"""  
        returnself._prompts.get(name)  
  
    deflist_prompts(self) -> list[Prompt]:  
        """列出所有已注册的提示词。"""  
        returnlist(self._prompts.values())
```
...
> 官方源码：utilities/func\_metadata.py
>
> **调试文件下载**：debug\_func\_metadata.py
...
import inspect  
import json  
from typing importAny, Callable, Sequence, Optional, List, Dict  
...
defprint_json(data, title="JSON数据"):  
    """打印JSON数据"""  
    try:  
        print(f"\n📄 {title}:")  
...
        # 步骤2: 遍历每个参数  
        processed_count = 0  
        for idx, param inenumerate(params.values()):  
...
                    None,  
                    Field(default=param.default if param.default isnot inspect.Parameter.empty else PydanticUndefined),  
                ]  
...
                typed_annotation,  
                param.default if param.default isnot inspect.Parameter.empty else PydanticUndefined,  
            )  
...
            # 例如：'dict' 或 'json' 等  
            ifhasattr(BaseModel, param.name) andcallable(getattr(BaseModel, param.name)):  
...
        # 基于返回类型注释设置结构化输出支持  
        if sig.return_annotation is inspect.Parameter.empty and structured_output isTrue:  
...
        # 模型创建失败或产生警告 - 无结构化输出  
        if output_model isNoneand structured_output isTrue:  
...
    # 前缀参数测试  
    print("\n\n📌 测试2: 前缀参数冲突")  
    defprefix_func(_private: str, field_test: int) -> str:  
        """前缀参数"""  
        return"test"  
    debug_func_metadata(prefix_func)  
  
    print("\n\n📌 测试3: 结构化输出对比")  
    defadd(a: int, b: int) -> str:  
        return a + b  
    print("📌 无结构化")  
