...
author: 宝玉AI
date:
url: https://mp.weixin.qq.com/s?__biz=Mzk1NzgxMjQ0OA==&mid=2247492197&idx=1&sn=b1190b9f2c74723b5533e430e4568142&chksm=c2f1930f87e0311c9136137a71aa0c87390c6e3220bea019a33500781841e82ec469e1c1ce62&mpshare=1&scene=24&srcid=0120r49wE5zXjxlJCxGMdMpL&sharer_shareinfo=43b475a90b5bbc5c264740c71943ad50&sharer_shareinfo_first=43b475a90b5bbc5c264740c71943ad50#rd
---
...
1. 1. 用户输入命令，比如 `/article-illustrator path/to/article.md --style tech`
2. 2. Agent 读取文章，分析结构，识别需要配图的位置。判断标准是：这个地方加张图，能帮助理解还是纯粹凑数？能帮助理解的留下，凑数的不要。
3. 3. 根据 `--style` 参数选风格。如果没指定，就扫描文章内容，按预设规则自动匹配。
4. 4. 生成配图方案，列出每张图的插入位置、目的、视觉内容、文件名。
5. 5. 为每张图写详细提示词，保存到 `imgs/prompts/` 目录。
6. 6. 逐张调用图像生成工具（我用的是 Gemini），生成失败会自动重试一次。
7. 7. 把生成的图片插入文章对应位置，格式是 `![描述](imgs/xxx.png)`。
8. 8. 输出总结：用了什么风格、生成了几张图、哪张插在哪里。
