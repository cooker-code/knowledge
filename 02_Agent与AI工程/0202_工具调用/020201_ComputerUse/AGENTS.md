...
- 本模块先看宏观流程，再看文章：[知识地图](020201_核心知识点/知识地图.md)。
- 新文章必须先归入流程节点，再判断是补充、冲突、不同层次还是降权。
- `文章/` 只保留原文锚点，长期知识必须沉淀到 `020201_核心知识点/`。
...
|---|---|---|---|---|
| 高权限 Computer Use 与沙箱边界 | [高权限ComputerUse与沙箱边界](020201_核心知识点/高权限ComputerUse与沙箱边界.md) | Computer Use + 视觉感知 + 坐标映射 + 设备 Operator + 沙箱/审批 + 本地文件和浏览器权限 | 判断 GUI Agent 什么时候值得用，什么时候必须降级或隔离 | Computer Use 是高权限执行层，不是普通工具调用能力 |
