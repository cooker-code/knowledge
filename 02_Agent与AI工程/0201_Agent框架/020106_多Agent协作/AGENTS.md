...
- 本模块先看宏观流程，再看文章：[知识地图](020106_核心知识点/知识地图.md)。
- 新文章必须先归入流程节点，再判断是补充、冲突、不同层次还是降权。
- `文章/` 只保留原文锚点，长期知识必须沉淀到 `020106_核心知识点/`。
...
|---|---|---|---|---|
| Supervisor 控制边界 | [多Agent协作的Supervisor控制边界](020106_核心知识点/多Agent协作的Supervisor控制边界.md) | 多 Agent 协作 + Supervisor/并行/降级/Writer + 任务分配和结果合并 + 生产可控性 + 避免把多个 LLM 调用误认为多 Agent 系统 | 多 Agent 如何处理分配、失败、矛盾和并发上限 | 多 Agent 的价值不在“Agent 数量”，而在决策权、失败隔离和合并规则显式化 |
