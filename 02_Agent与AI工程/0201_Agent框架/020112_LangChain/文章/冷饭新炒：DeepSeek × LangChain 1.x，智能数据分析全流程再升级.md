---
title: 冷饭新炒：DeepSeek × LangChain 1.x，智能数据分析全流程再升级
author: 翊行代码
date: 
url: https://mp.weixin.qq.com/s?__biz=MjM5NTQ3NDAwMw==&mid=2452917949&idx=1&sn=bfad23f1cb8bd272aa44668bf16e3280&chksm=b0cbbeafa66b39bd83d9ff79ee6d5d8117615597e92e472eef0ea310f1468a11d94e461d7a33&mpshare=1&scene=24&srcid=1211dSrLJ2Pz1bTtGUA9ty81&sharer_shareinfo=570217dbcac510c5567486250c2694fb&sharer_shareinfo_first=570217dbcac510c5567486250c2694fb#rd
---
...
1. 1. 核心 Agent 抽象收敛到 `create_agent` + middleware
2. 2. 不同厂商的模型输出收敛到标准内容块 `content_blocks`
3. 3. 命名空间瘦身：`langchain` 保留核心积木，历史功能挪到 `langchain-classic`
...
## 二、三大核心变化：create\_agent / content\_blocks / 命名空间瘦身

### 2.1 create\_agent：Agent 构建方式的“收口”
...
@tool  
defsearch_product(product_name: str) -> str:  
    """搜索某个商品的销量信息"""  
    # 这里可以接真实搜索 / 数据查询逻辑  
    returnf"找到 {product_name} 的销量数据：......"  
...
```
工具（Tools）ChatDeepSeekdeepseek-reasonerMiddlewareAgent（create_agent）同学 / 业务代码工具（Tools）ChatDeepSeekdeepseek-reasonerMiddlewareAgent（create_agent）同学 / 业务代码发送 messagesbefore_agent / before_model调整后的请求模型响应（含工具调用建议）after_model 校验 / 记录执行工具调用工具结果最终回复（messages）
```
...
为了让迁移更“有血有肉”，我们选用第 06 篇《[DeepSeek R1 × LangChain：智能数据分析全流程实践](https://mp.weixin.qq.com/s?__biz=MjM5NTQ3NDAwMw==&mid=2452917691&idx=1&sn=591241c1b1118949af174219d9f435b2&scene=21#wechat_redirect)》里的核心逻辑来做例子。
...
```
from typing importOptional, Dict, Any  
...
defanalyze_data(data_sample: str) -> Optional[Dict[str, Any]]:  
...
1. 1. 使用 `response.text` 而不是 `response.content`

* • 1.x 中 `.text()` 已经变为 `.text` 属性
* • `.text()` 继续可用，但会给 deprecation warning，未来版本会移除

2. 2. 添加 `output_version="v1"`
...
### 3.3 迁移步骤 2：用 create\_agent 封装数据分析链路（可选进阶）
...
@tool  
defexecute_matplotlib(code: str) -> str:  
