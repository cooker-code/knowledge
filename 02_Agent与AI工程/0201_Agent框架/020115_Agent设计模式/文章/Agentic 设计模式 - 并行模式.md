---
title: Agentic 设计模式 - 并行模式
author: 羊八井花园
date: 
url: https://mp.weixin.qq.com/s?__biz=MzA4ODU5NDIyNQ==&mid=2456448176&idx=1&sn=bd503447310a1c125a496eb9d183a374&chksm=862b91e627458df4b35b76329a479e6cf944254cbdae814f151ccc208835b4a464e18d0a4740&mpshare=1&scene=24&srcid=0919sMFGuRrdJKfo0qVbWGNp&sharer_shareinfo=062997fe20de70bae3f7120eaa525393&sharer_shareinfo_first=062997fe20de70bae3f7120eaa525393#rd
---
...
3. 3. 搜索资料来源 B。
4. 4. 总结资料来源 B 的内容。
5. 5. 基于 A 和 B 的摘要整合出最终答案。

而采用并行方法则可以优化为：

1. 1. 同时搜索资料来源 A 和资料来源 B。
2. 2. 两项搜索都完成后，并行总结资料来源 A 和资料来源 B 的内容。
3. 3. 基于 A 和 B 的摘要整合出最终答案（此步骤通常需要顺序执行，需等待前面的并行步骤全部完成）。
...
1. 1. **信息采集与研究**：并行从多源获取信息，如新闻、社交媒体、数据库等。
...
2. 2. **数据处理与分析**：并行应用多种分析技术或处理不同数据片段。
...
3. 3. **多 API/工具交互**：调用多个独立 API 或工具以获取异构信息或执行不同操作。
...
6. 6. **多模态处理**：并行处理同一输入的不同模态（文本/图像/音频）。
...
7. 7. **A/B 测试或多方案生成**：并行生成多种响应变体以择优选取。
...
1. 1. **摘要链：** 使用定制化系统提示与用户提示（含主题占位符）生成精简摘要
2. 2. **问题链：** 针对主题生成三个关联性提问
3. 3. **关键词链：** 提取 5-10 个核心术语并以逗号分隔输出
