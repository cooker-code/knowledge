---
title: Agentic 设计模式 - 路由模式
author: 羊八井花园
date: 
url: https://mp.weixin.qq.com/s?__biz=MzA4ODU5NDIyNQ==&mid=2456448170&idx=1&sn=795022e24f762d1232f79ed3832ce011&chksm=8666d45dd37d686bc28fac3203abe15a9afcf0067c57965b9b323d46b31d5b83b9e29627837e&mpshare=1&scene=24&srcid=0911mUWzfakJR8nzorSI6ntJ&sharer_shareinfo=6300ec068a22140230609b1f504e03e1&sharer_shareinfo_first=6300ec068a22140230609b1f504e03e1#rd
---

本文示例代码见：routing-langgraph.py[1]，ch02-routing.rs[2]
...
LangChain[3]/LangGraph[4], rig.rs[5] 等计算框架提供了定义和管理此类条件逻辑的显式构造。LangGraph 基于状态图架构，特别适用于决策取决于系统累积状态的复杂路由场景。同样，谷歌 ADK 提供了构建智能体能力和交互模型的基础组件，这些组件可作为实现路由逻辑的基础。在这些框架提供的执行环境中，开发者可定义可能的运行路径，以及决定计算图中节点间转换的函数或模型评估机制。
...
1. 1. 类型安全 - Rust 的类型系统确保路由决策和状态转换在编译时就能得到验证
2. 2. 内存安全 - Rust 的所有权模型可以防止并发路由场景中的内存泄漏和数据竞争
3. 3. 性能优势 - 零成本抽象和无垃圾回收机制为路由操作提供更好的吞吐量
4. 4. 会话管理 - 内置的会话存储功能可以在路由决策过程中维护状态（注：LangGraph 也支持会话管理）
5. 5. 异步支持 - 一流的异步支持可以高效处理并发路由请求
...
#[async_trait]  
impl TaskforCoordinatorRouterTask {  
...
  async fn run(&self, context: Context) -> graph_flow::Result<TaskResult> {  
    letrequest: String = context.get_sync("request").ok_or_else(|| anyhow::anyhow!("'request' 不存在"))?;  
...
`[1]` routing-langgraph.py: *https://github.com/yangjing/learn-ai/blob/main/agentic/adp/ch02/routing-langgraph.py*  
`[2]` ch02-routing.rs: *https://github.com/yangjing/learn-ai/tree/main/agentic/adp/examples/ch02-routing.rs*  
`[3]` LangChain: *https://docs.langchain.com/oss/python/langchain/overview*  
`[4]` LangGraph: *https://docs.langchain.com/oss/python/langgraph/overview*  
`[5]` rig.rs: *https://docs.rs/rig-core/latest/rig/*