> 已吸收至：[[02_Agent与AI工程/0209_Harness Engineering/0209_核心知识点/上下文与工具加载边界|上下文与工具加载边界]]、[[02_Agent与AI工程/0209_Harness Engineering/0209_核心知识点/知识沉淀与私域资产|知识沉淀与私域资产]]
---
title: 技术教科书：顶级开发团队设计的Harness工程项目源码什么样
author: 腾讯技术工程
date:
url: https://mp.weixin.qq.com/s?__biz=MjM5ODYwMjI2MA==&mid=2649801110&idx=1&sn=27dac2e66ec72e89e07eeddcaa4614d8&chksm=bfec018461a64930820660bc27f6216f3630ee622d68f365554b38b06771af0437d35644a2a9&mpshare=1&scene=24&srcid=0410qrwPM7IE56TXP0YPzszV&sharer_shareinfo=8f23138a36c6a1a1e9b5c4685f261419&sharer_shareinfo_first=8f23138a36c6a1a1e9b5c4685f261419#rd
---
...
| 模块 | 大小 | 策略 |
| --- | --- | --- |
| OpenTelemetry | ~400 KB | 延迟导入，到遥测初始化时才加载 |
| gRPC (grpc-js) | ~700 KB | OpenTelemetry exporter 进一步延迟 |
| React/Ink 渲染 | 大量 | 非交互模式（`-p`）完全不加载 |
| 内部命令模块 | `insights.ts` 113KB | 懒加载 shim，用户执行 `/insights` 时才加载 |
| Sentry | 较大 | `is_feature_enabled()` 特性开关，外部构建剥离 |
...
| 熵治理手段 | 该项目实现 | 触发条件 |
| --- | --- | --- |
| **上下文蒸馏** | `/compact` + Auto Compact | 手动或上下文接近窗口限制 |
| **知识沉淀** | `memdir/` 持久化写入 | Agent 主动调用 |
...
| **编译时 DCE** | 整个代码块 | `feature()` + Bun bundler 剥离内部工具 |
| **memoize 单例** | 重复调用 | `getCommands()` , `getUserContext()` 只算一次 |
| **API 预连接** | ~50ms | `preconnectModelApi()` TLS 预握手 |
| **缓存排序** | 真金白银 | 工具列表分区排序保 prompt cache 命中率 |
| **流式并行** | 整体延迟 | `StreamingToolExecutor` 边流式边执行 |
...
| 属性 | 英文 | 含义 |
| --- | --- | --- |
| 🐛 调试 | DEBUGGING | 这只精灵擅长找 bug |
| ⏳ 耐心 | PATIENCE | 它能忍受你的代码 |
| 🌪️ 混乱 | CHAOS | 它有多疯狂 |
| 🧙 智慧 | WISDOM | 它的知识水平 |
| 😏 毒舌 | SNARK | 它说话有多刻薄 |
