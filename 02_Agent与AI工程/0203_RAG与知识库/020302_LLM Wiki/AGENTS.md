...
- 本模块先看宏观流程，再看文章：[知识地图](020302_知识地图.md)。
- 知识地图放在三级节点根部；`020302_核心知识点/` 只放长期主题页。
- 新文章必须先归入流程节点，再判断是补充、冲突、不同层次还是降权。
- `文章/` 只保留原文锚点，长期知识必须沉淀到 `020302_核心知识点/` 下的主题文件。
...
|---|---|---|---|---|
| LLM Wiki 预编译知识库模式 | [LLMWiki预编译知识库模式](020302_核心知识点/LLMWiki预编译知识库模式.md) | LLM Wiki + Ingest/Query/Lint + 预编译 Markdown 网络 + 个人知识库沉淀 + 与 RAG/knowledge 边界 | 判断旧 wiki 中哪些机制应吸收到新 knowledge | LLM Wiki 的优势是互链和健康检查，knowledge 的优势是认知校准和阅读投入判断，两者应合并而不是二选一 |
| Git 预编译知识库与 Lint 闭环 | [Git预编译知识库与Lint闭环](020302_核心知识点/Git预编译知识库与Lint闭环.md) | LLM Wiki + Git/Markdown/CI/Lint/成熟度/引用追踪 + 预编译知识库治理 + 改进 knowledge 初始化闭环 | 把 LLM Wiki 思想落到版本化、审查、质量门禁和上下文注入 | 当前 knowledge 应吸收 lint、三级索引、成熟度和引用追踪，而不是照搬全量 wikilink |
...
- 已归档来源：`5` 篇，全部位于 `文章/` 且使用 `done-` 前缀。
- 核心知识点总览：[LLMWiki知识编译与治理边界.md](020302_核心知识点/LLMWiki知识编译与治理边界.md)，承接来源锚点和粗分流记录。
