> 已吸收至：[[04_OLAP与数据库/0404_搜索与索引/040401_Elasticsearch/040401_核心知识点/Elasticsearch地理空间索引边界|Elasticsearch地理空间索引边界]]
---
title: 干货｜Elasticsearch 在地理信息空间索引的探索和演进
author: 快乐马AI
date:
url: http://mp.weixin.qq.com/s?__biz=Mzg3NTI2MzMzMw==&mid=2247496393&idx=1&sn=84d012e716da3358a0a45986488dae7e&chksm=cec696c3f9b11fd5aed474f61de81a2fefcdf598a89490396034612e4ac7ab894ca59f025de5&mpshare=1&scene=24&srcid=0714dCP7onWZ3A0Vl573bwOd&sharer_sharetime=1657781912329&sharer_shareid=ac11efa86a18b42da87220879062c874#rd
---
...
**第一步:** 通过关键词找到对应的倒排表。这一步简单来说就是查词典。例如：

TermQuery.TermWeight 获取该term的倒排表，读取docId+freq信息。
...
Lucene提供了一种适配方案RangeQuery。就是用枚举来模拟数值查询。简单来说：

RangeQuery=BooleanQuery+TermQuery，所以限制查询是整数且区间最大不能超过1024。这种实现是可以说是非常鸡肋的，好在Lucene 2.9.0版本真正支持数值查询。
...
这个实现很强大，支持了int/long/float/double/

short/byte，也不限制查询区间了。它的核心思路是将数值字节数组化，然后利用前缀分层管理区间。

 如下图所示：

本质上还是RangeQuery=

BooleanQuery+TermQuery，只不过在前面做了一层转换：通过前缀树管理一个区间实现了匹配词数量的缩减，而这个缩减是非常有效的。所以这里就有一个专家参数：precisionStep。就是用来控制每个数值字段在分词是生成term的数量，生成term数量越多，区间控制粒度越细，占用磁盘空间越大，查询效率通常越高。
...
ES2.0版本的实现有个问题， 就是没有很好解决二维组合条件查询的数据筛选。它是分别获取**符合纬度范围条件的文档集合**和**符合经度范围条件的文档集合**然后进行交集，初筛了太多无效的文档集合。
...
例如：ES 2.2版本对于geo\_distance的实现关键点，判断索引版本是否是V\_2\_2\_0版本以后创建，如果是则直接用Lucene的

GeoPointDistanceQuery查询类，否则沿用ES 2.0版本的GeoDistanceRangeQuery。
...
对于CELL\_CROSSES\_QUERY，如果是叶子节点，则需要判断cell中的每个POI是否符合query的查询条件；否则查询子区间；

对于CELL\_OUTSIDE\_QUERY，直接略过；

对于CELL\_INSIDE\_QUERY，整个cell中的POI都满足查询条件。

**核心代码：**

LatLonPoint/LatLonPointDistanceQuery
