> 已吸收至：[[04_OLAP与数据库/0406_存储引擎/040601_FlowDB/040601_核心知识点/FlowDB时序存储引擎待验证边界|FlowDB时序存储引擎待验证边界]]
---
title: FlowDB 架构解析：Rust 的时序存储引擎为什么能比C++的RocksDB快好几倍
author: 入职啦
date: 路奇路奇
url: https://mp.weixin.qq.com/s?__biz=MzkwOTU4MTg5Nw==&mid=2247486872&idx=1&sn=6aee41a539beeb1ff0db2432774862f6&chksm=c09c6798bb66a50e12ec461152eb452f055f83b41d555bb143230191e553785e081a0ff3a210&mpshare=1&scene=24&srcid=0603RNUP9djpZElsSTDrHTtl&sharer_shareinfo=d1930d90788d41c014e51e951b9f5505&sharer_shareinfo_first=d1930d90788d41c014e51e951b9f5505#rd
---

# FlowDB 架构解析：一个 Rust 时序存储引擎为什么能比RocksDB快好几倍

FlowDB是用Rust编写的高性能时序数据库, 代码在 https://github.com/restsend/flowdb
...
* `engine.rs`
  ...
  统一入口，负责写入、查询、扫描、flush、compaction、gc。
* `wal.rs`

  预写日志，负责崩溃恢复。
* `memtable.rs`

  内存表，分 active 和 frozen 两层。
* `sstable.rs`
...
* `bloom.rs`
  ...
  Bloom Filter，用于快速判断 key 是否不在某个 SST 中。
* `cache.rs`
...
* `Arc`
  ...
  让 reader 和 cached block 共享变得便宜且安全。
* `AtomicU64`
