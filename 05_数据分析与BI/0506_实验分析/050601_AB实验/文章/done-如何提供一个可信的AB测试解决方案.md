...
author: 美团技术团队
date:
url: http://mp.weixin.qq.com/s?__biz=MjM5NjQ5MTI5OA==&mid=2651774833&idx=1&sn=fbbec16c014d295afa29dc77b0b55e08&chksm=bd12063c8a658f2a542953808a122b2ce8f2b066eafa787bfcd21c7fa2b6844523607d0d57ee&mpshare=1&scene=24&srcid=0831EbjBaRmyvSJP8KXz0GrH&sharer_shareinfo=64c9c0e38ec3476c190f53313c65f8df&sharer_shareinfo_first=64c9c0e38ec3476c190f53313c65f8df#rd
---
...
样本在实验组或对照组的分配机制影响着其方差的计算，避免用随机抽样（样本满足独立同分布）下的样本方差计算方法计算非随机抽样下的样本方差。该陷阱主要出现在用随机对照实验下的方差计算方法计算准实验下的方差，导致方差计算错误。计算的方差，按照方差性质，非随机抽样下，若和不独立：
...
例如对于骑手运单量指标可通过计算实验组与对照组的样本均值差来估计实验效应，并构造两样本t-统计量对原假设实验组对照组无差别进行双边假设检验。为方便讨论，先不考虑随机性，仅考虑分组随机性， 并且假设实验组样本量，对照组样本量固定。

样本在实验组，样本在对照组，令，与分配机制有关，若：

则：

若想保证为真实效果的无偏估计，实际上分配机制要保证如下公式成立：

实际中，如果分配机制保证相和性，在原假设下，方差计算为：
...
这是错误的，因为分母  本身也是一个随机变量。另外当分析单元比实验单元更细时，例如，骑手有单均时长指标，在骑手活动的实验中，实验单元为骑手而骑手单均时长指标（所有运单时长的平均值）的分析单元为运单，分析单元为比实验单元更细的方差时，[Deng等(2011)](https://alexdeng.github.io/public/files/jsm2011-deng.pdf)与[Deng等(2016)](https://alexdeng.github.io/public/files/WSDM2017draft.pdf)指出此时分析单元样本间的独立性不再成立，因为运单1、2、3来自同一个骑手的相关行为时，运单时长指标、、应该是相关的。此时应重写样本均值为：

此时的方差应为：

其中为第个骑手的运单时长之和，为第个骑手的运单量，为骑手个数。另外对于比率型指标等更复杂的指标，正确的方差估计公式往往需要统计学中的Delta方法等推导得到。
...
从经验来看，若样本仅稍微偏离正态总体，大于30的样本量或许就足够了。然而对于有偏样本，[Ron Kohavi等(2014)](https://www.exp-platform.com/Documents/2014%20experimentersRulesOfThumb.pdf)  指出当样本偏度大于等于1时，一个经验准则便是只有计算样本均值的观测样本量大于时才可认为中心极限定律成立。实际抽取了一个样本量为13832的活动实验，其实验组、对照组差值的抽样分布呈现右偏，不符合正态分布，如下图所示：
...
**|**[数据治理一体化实践之体系化建模](http://mp.weixin.qq.com/s?__biz=MjM5NjQ5MTI5OA==&mid=2651766711&idx=1&sn=f45ba9ee2de591f9eec6b989dc9fbb2b&chksm=bd1266fa8a65efec108329afdd0060b108516657e5a266af61e803cad9e55c737a86e4156748&scene=21#wechat_redirect)

**|** [美团配送数据治理实践](http://mp.weixin.qq.com/s?__biz=MjM5NjQ5MTI5OA==&mid=2651751418&idx=2&sn=11f4d11b492c3390d99044a279ca69f2&chksm=bd125ab78a65d3a159d6e722e87d650d1ba5745f0ff0dbbb11e3036983b75982dce2a9e6cdcb&scene=21#wechat_redirect)

**|** [业务数据治理体系化思考与实践](http://mp.weixin.qq.com/s?__biz=MjM5NjQ5MTI5OA==&mid=2651768670&idx=1&sn=58931922b9899875702889ce58c0aad7&chksm=bd121e138a659705ceeaf12010586c227173587aefaba8cc4e1981e8272031aa56036b5a2ff8&scene=21#wechat_redirect)