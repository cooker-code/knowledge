---
title: LangChain v1 Agent：DeepSeek-V3.2/V3.2-Speciale在ChatBI里动态选模
author: 翊行代码
date: 
url: https://mp.weixin.qq.com/s?__biz=MjM5NTQ3NDAwMw==&mid=2452918000&idx=1&sn=ee6f2d96ef74058335188b11ae2b991b&chksm=b07ebb4319f4ebbd81b195628144374a02765b6163a928d736276603faa3a29217da6ff26238&mpshare=1&scene=24&srcid=1220iQMnOwkKFfdFGfSdQlgF&sharer_shareinfo=39f401c36197bcd700d63461b59b40b5&sharer_shareinfo_first=39f401c36197bcd700d63461b59b40b5#rd
---
...
1. 1. **数据获取（必须能调工具）**：把自然语言问题变成 SQL，并执行查询拿到数据
2. 2. **分析与验证（需要更强推理，但不一定要工具）**：基于数据做推导、对齐口径、发现异常、给出结论与依据
3. 3. **表达与收口（偏格式化）**：把结论写成“业务能看懂”的回答（可控长度、结构清晰）
...
1. 1. **工具环节**：永远用“支持 tool calls”的模型（例如 V3.2）
2. 2. **分析环节**：如果你开启了“严格分析”，就把工具禁用，然后切到 Speciale（避免它尝试 tool call）
...
@wrap_model_call  
defroute_model(request: ModelRequest, handler: Callable[[ModelRequest], ModelResponse]) -> ModelResponse:  
...
1. 1. **每档模型用一致的绑定方式**（强/便宜都 bind 同一组能力，保持可替换）
2. 2. **把绑定推迟到调用链里**（在路由后，再对选中的 model 进行绑定）
